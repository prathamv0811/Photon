...
from photon.config import validate_lerobot_config, create_robot_configs, get_known_ids, save_lerobot_config, get_robot_config_classes, create_follower_config
from photon.calibration import display_calibration_error, display_arms_status
from photon.auth import authenticate_huggingface
from photon.dataset import check_dataset_exists, handle_existing_dataset, normalize_repo_id
    ...
    **mode_specific_kwargs
    ...
    if mode == "recording":
        ...
        if repo_id.startswith('local/') and push_to_hub:
            ...
            from photon.auth import get_stored_credentials
...
def training_mode(config: dict, auto_use: bool = False):
    ...
    if preconfigured:
        ...
        # Step 1: HuggingFace Authentication (Required for most workflows)
        typer.echo("\n🔐 Step 1: HuggingFace Authentication")
        login_success, hf_username = authenticate_huggingface()
        if not login_success:
             typer.echo("⚠️  Continuing without HuggingFace authentication (some features may be limited).")
        ...
        typer.echo("7. SARM (State-Action Reward Model)")
        ...
        policy_name = policy_name_map.get(policy_choice, "diffusion") # Default to diffusion if invalid
        ...
        # Step 5: Hub pushing configuration
        typer.echo(f"\n🚀 Step 5: HuggingFace Hub Configuration")
        ...
        # Step 6: WandB logging configuration
        typer.echo(f"\n📊 Step 6: Weights & Biases Configuration")
    ...
    # Import training function from our custom trainer module
    from photon.trainer import train
    ...
    try:
        ...
        if pretrained_policy_path:
            ...
            elif policy_name == "sarm":
                # SARM requires specific configuration often dependent on the dataset
                # We'll use default for now but warn user
                typer.echo("⚠️  SARM policy selected. Ensure your dataset supports it.")
                try: 
                    from lerobot.policies.sarm.configuration_sarm import SARMConfig
                    policy_config = SARMConfig()
                except ImportError:
                     # Fallback if sarm isn't directly importable or named differently
                    typer.echo("❌ Could not import SARMConfig. Please check LeRobot version.")
                    raise
            else:
